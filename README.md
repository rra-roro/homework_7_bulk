...
`-N <size>`   - **обязательный параметр**, задающий число команд в блоке.
...
Моя реализация фактически является аналогом идеи Herb Sutter и имеет те же проблемы, что и проблемы, 
...
2)	Таким образом, подписчиками могут быть любые ф-ии и функциональные объекты,
...
3) В классе **publisher_mixin** так же добавлена возможность отписаться от уведомления.<br>
...
4)	> **lvalue** функторы сохраняются по ссылке, что позволяет из ф-ии уведомления
...
5)  Особенности **publisher_mixin**<br>
...
6)	В целях снижения связности publisher класс сделан в виде mixin класса:  **publisher_mixin**<br>
...
7) Если ф-ия подписчика возвращает значение, то ф-ию `notify()`собирает эти значения и возвращает
...
8) Обраотка исключений организована в стиле С++11.
